describe Sinatra::Application do
  ...
  let(:records) { create_list :city, 3, state: %w[FL NY].sample }
  let(:default_presentation) { { except: [:created_at, :updated_at] } }
  let(:result) { Presenters::Resource.present(Cities::Repository.get, default_presentation) }
  ...
  describe 'GET /cities' do
    ...
    context 'passing query options' do
      let(:route) { '/cities?state=FL&limit=2&order=name:desc&only=name,state&page=1' }
      let(:data_query) { { state: 'FL', limit: 2, order: {name: :desc}, page: 1 } }
      let(:presentation_query) { { only: [:name, :state] } }
      let(:result) { Presenters::Resource.present(Cities::Repository.get(data_query), presentation_query) }
  ...
  describe 'GET /cities/:id' do
    ...
    let(:route) { "/cities/#{record.id}" }
    let(:relations) { {include: {companies: {only: [:id, :name]}}} }
    let(:result) { Presenters::Resource.present(
        Cities::Repository.get(filter: { id: record.id }).preload(:companies),
    ...
    context 'passing query options' do
      let(:route) { "/cities/#{record.id}?only=name" }
